...
```{r}
exemple <- c("Alice", "Bob", "Caroll", "Mary Caroll")
```
...
Comme une expression mathématique : on combine des expressions simples (des "blocs") pour en faire des expressions plus complexes.
...
```{r}
grep(pattern = "o", x = letters, value = TRUE)
```

Ou une classe de caractères entre `[]` :

```{r}
grep(pattern = "[bxe]", x = letters, value = TRUE)
```
