...
`infert` est recherché dans l'environnement où `f()` est définie. Si on appelle `f()` depuis une fonction où on défini un autre jeu de données `infert` ça ne fonctionnera pas.

Par exemple on veut faire une fonction qui rééchantillonne `infert` et calcule la force d'association avec `case`.
...
Car le `infert` utilisé par la fonction `assoc_case_infert()` est toujours celui de son *enclosing environment*, l'environnement global, et pas l'`infert` de son *calling environment*, l'*execution environment* de `resample_and_test()`.
...
Faire un fonction qui prend comme argument un nombre de points et représente un nuage de points aléatoirement généré selon un distribution normale. Les paramètres de `plot()` doivent pouvoir être modifiés depuis la fonction.
...
Plus avancé, utilise `lapply()` (prévu pour la leçon suivante). Faire une fonction qui à partir d'un nombre arbitraire de `t.test()` retourne le plus significatif.
