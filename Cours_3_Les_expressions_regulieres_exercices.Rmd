...
```{r result = 'asis'}
ref <- read.csv2(
  '~/Documents/r_isp/FINESS_data.csv',
...
finMatch <- sapply(finMatch, f_nettoy)
```
...
```{r}
finSub <- gsub(pattern = ".+([[:digit:]][[:digit:]|A|B][[:digit:]]{7}).+", replacement = "\\1", ref$info)
```
