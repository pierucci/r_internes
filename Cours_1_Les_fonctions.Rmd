...
En `R` les noms d'objets (`x`, `varName`...) sont liés à des valeurs (`3`, `"foo"`...). Ces informations sont contenues dans un *environement* (une sorte de "sac de noms").

Par exemple l'environement où on travaille quand on ouvre `R` est l'environement global, `globalenv`. Mais il peut exister d'autres environements.

Chaque fonction utilise 3 environements : enclosing, execution, calling.

## Execution environment

Quand une fonction est lancée, le code contenu dans le corps est éxécuté dans un nouvel environement, créé *de novo*.

Sauf exception, cet environement est supprimé et les données sont perdues quand la fonction est finie.
