...
```{r}
monolith <- function(data, col = "blue", pch = 21) {
        # transform data
        # fit model to data
        # plot data, uses 'col' and 'pch'
        # get desired model results
...
C'est l'environnement où la fonction a été *définie*. Pour une fonction définie classiquement, c'est donc l'environnement global. Mais il est possible que ça soit un autre environnement, par exemple l'*execution environment* d'une autre fonction (quand une fonction est définie dans une autre fonction).
...
f()
```
Mais pour cette même fonction ça peut aussi être l'*execution environment* de `g()`.
```{r}
g <- function() {
  f()
}
...
- faire passer des arguments pour une fonction exécutée à l'intérieur d'une autre fonction, sans avoir à tous les nommer.
```{r}
...
## Gestion des valeurs de retour complexes (1)
...
## Gestion des valeurs de retour complexes (2)
...
## Pour combiner les concepts encore plus

Il faut remonter d'un cran l'*enclosing environment*.
```{r}
# local() permet de créer un environment
