...
C'est l'environnement où la fonction a été *définie*. Pour une fonction définie classiquement, c'est donc l'environnement global. Mais il est possible que ça soit un autre environnement, par exemple l'*execution environment* d'une autre fonction (quand une deuxième fonction est définie dans une première fonction).
...
f()
```
Mais pour cette même fonction, si elle est lancée depuis `g()`, ça peut aussi être l'*execution environment* de `g()`.
```{r}
g <- function() {
  f()
}
