...
Quand une variable n'est pas définie directement dans l'environnement de la fonction, R va la chercher ailleurs...

Plus précisement, R va remonter progressivement le `searchPath` à la recherche d'une correspondance avec une variable/fonction portant ce nom.
...
## Le lexical scoping en détails (1/4)
Ici, la variable `x` n'est pas définie dans l'environnement de la fonction, mais dans l'environnement global. 

A l'exécution de la fonction `f`, R va d'abord chercher une correspondance pour `x` dans l'environnement de la fonction, puis va remonter jusqu'à la trouver dans l'environnement global.
```{r}
x <- 10
f <- function() {
  print(x)
...
Fonctions et variables nommées ne sont pas identiques (quand bien même le nom est identique).
Ainsi, il est possible de manipuler des fonctions et des variables avec le même nom.

**ATTENTION : pratique, mais c'est une potentielle source d'erreurs...**
...
Les variables encapsulées dans l'environnement de la fonction n'existent et/ou ne sont manipulées que pendant la durée de l'exécution de la fonction, puis disparaissent à la fin de l'exécution.
...
## Le lexical scoping en détails (4/4)
