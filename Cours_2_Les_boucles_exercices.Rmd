...
Si on souhaite obtenir des données à partir d'une année précise, un approche est de travailler sur un subset d'une année. Par exemple pour `pce`, personal consumption expenditures, in billions of dollars :
...
Pour appliquer ligne à ligne une fonction sur une table (ou colonne par colonne).
...
Il est possible d'utiliser une fonction qu'on a écrit avec `apply`. ^[Avec `lapply` aussi d'ailleurs.] Par exemple pour compter les données manquantes par variables :
