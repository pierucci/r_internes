...
Ensemble de packages autour de la manipulation de données développés par/avec Hadley Wickham. Ces packages sont tous développés de façon à être:
  ...
  * le plus cohérent possible (= convention de nommage, cohérence des arguments)
...
## Quelques packages
  ...
  - `tidyr` : dédié à la mise en forme + nettoyage des données (format long/wide)
...
Plus d'info : `browseVignettes(package = "magrittr")`.
...
On peut combiner le chaînage avec le retour à la ligne et avec `->` pour plus de lisibilité.
...
tab %>% 
  summarise(
    ...
    var3 = sum(var))
```
Il faut donc utiliser des fonctions qui prennent un vecteur avec plusieurs valeurs, et retournent une valeur unique. On peut appeler ces fonctions des *fonctions de résumé (summary functions)*.
...
Il existe quelques fonctions spéciales très utiles: `n()` le nombre de lignes, `row_number()` le numéro de ligne, `lead()` et `lag()` pour récupérer les lignes suivantes ou précédentes `n_distinct()` pour compter le nombre distinct d'éléments.
```{r}
tab %>%
  mutate(
    nline = n(),
    ...
    lead_var = lead(var),
    lag_var = lag(var),
    n_gpe = n_distinct(class))
```
*N.B. : on peut aussi évoquer `n_distinct` qui compte le nombre distinct d'éléments (un cas concret : on peut compter un nombre de séjours en comptant le nombre de ligne (fonction `n`) et compter le nombre de patients avec `n_distinct` sur la variable identifiant les patients, un patient pouvant effectuer plusieurs séjours)*

## Fonctions spéciales + `group_by()` {.reduceCode}
